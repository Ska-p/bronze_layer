...
def stream_targz_extraction(container, raw_path, dest_path):
    blob_client = container.get_blob_client(raw_path)
    raw_data = blob_client.download_blob()
    extracted_paths = []
    with gzip.GzipFile(fileobj=io.BytesIO(raw_data)) as gz:
        with tarfile.TarFile(fileobj=gz) as tar:
            for member in tar.getmembers():
                ...
                dest_path = f"{dest_path}/{member.name}"
                
                logger.info(f"Streaming decompression (.tar.gz): {raw_path} -> {dest_path}")
                
                member_file = tar.extractfile(member)
                container.upload_blob(dest_path, member_file, overwrite=True)
...
def extract(source_id: str, container: ContainerClient):
    ...
    try:
        for file_path in entry["list_of_files"]:
            ...
            if file_path.lower().endswith(".tar.gz"):
                dest = file_path.replace("raw/", "extracted/", 1).split("/")[:-1]
                paths = stream_targz_extraction(container, file_path, dest)
                all_extracted.extend(paths)
                
            elif file_path.lower().endswith(".gz") and not file_path.lower().endswith(".tar.gz"):
...
if __name__ == "__main__":
    ...
    extract(source_id=args.id, container=container)